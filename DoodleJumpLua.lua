...
function on.timer()
    ...
    if doodle.overrideTerrainScroll > 2 then doodle.overrideTerrainScroll = doodle.overrideTerrainScroll - .1 end

    doodle.height = doodle.height + .5
...
function Doodle:moveY(step)
    ...
    elseif self.y > pwh-10 then
        ...
        if self.height > tmpHS then var.store("highscore", self.height) end
        timer.stop()
    end
end
...
function Texts:paint(gc)
    gc:setColorRGB(0,0,0)
    gc:drawString(math.ceil(doodle.height), 3, 0, "top")
    if gameOver then
        gc:setFont("sansserif", "b", 10)
        gc:drawString("Game over !", .5*(pww-gc:getStringWidth("Game over !")), pwh-45, "top")
        gc:setFont("sansserif", "r", 10)
        gc:drawString("ESC/Click to restart", .5*(pww-gc:getStringWidth("ESC/Click to restart")), pwh-25, "top")
    end
end
...
function Platform:init(x, y, theType)
    ...
    self.state = 1 --  TODO: states a déterminer.
    self.toDestroy = false
...
function Platform:paint(gc)
    ...
    elseif self.theType == "boost" then
        gc:drawImage(boostImg, self.x, self.y+2)
    end
end
...
function on.mouseMove(x,y)
    if not gameOver then doodle:setPos(x) end
    platform.window:invalidate()
end
